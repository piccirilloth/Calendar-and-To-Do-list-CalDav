...
todo_information::todo_information(QWidget *parent, std::string summary, std::string createdOn, std::string expDate, std::string completed, int priority) :
    ...
    ui->lineEdit_expDate->setText(QString(expDate.c_str()));
    if(completed == "0000/00/00")
        ui->lineEdit_completed->setText(QString("Not completed"));
    else
        ui->lineEdit_completed->setText(QString(completed.c_str()));
    if(priority == -1)
